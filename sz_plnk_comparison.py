...
d1 = pickle.load(gzip.open('data_sp/sptsz/data/spt/sz_cutouts.pkl.gz'))
#d1 = pickle.load(gzip.open('data_sp/sptsz/data/spt/temp_random_cutouts.pkl.gz'))
cts = d1['cutouts']
keys = cts.keys()
...
Bl_sz = sims.fn_beam_stuff(simmapparams, use_beam = 1, exp_beam = sptpol_beam, return_beam = 1)[0]
Bl_smoothing = Bl_planck/Bl_sz
Bl_smoothing[np.where(np.isnan(Bl_smoothing))] = 0.
Bl_smoothing[Bl_smoothing == np.inf] == 0.
Bl_ratio = Bl_smoothing
dx = 0.5
xs_cutout = 200
...
ra0,dec0 = 180.,0.
nside = 2048
plnk_cts = pickle.load(gzip.open('data_sp/sptsz/data/LGMCA/projection_corrected_lgmca.pkl.gz'))['cutouts']
#plnk_cts = pickle.load(gzip.open('data_sp/sptsz/data/LGMCA/temp_random_cutouts.pkl.gz'))['cutouts']
...
while j  < 3:
	...
	temp = plnk_cts[keys[i]][0]

	ra, dec = keys[i][0],keys[i][1]
	temp = temp/1e6
	map1= np.fft.ifft2( np.fft.fft2(pol_map) * Bl_ratio*TF* TWODTF).real
	map2= np.fft.ifft2( np.fft.fft2(temp)  * TWODTF*TF).real
	from pylab import *
	subplot(121);imshow(map1);colorbar()
	subplot(122);imshow(map2);colorbar()
	show()


	sys.exit()
	sptpol_gnom = H.gnomview(lgmca_map, rot = [ra, dec ],xsize = xs_cutout, coord = ['G','C'], reso = dx, return_projected_map = 1)
	close()
...
#			temp[j,k] = lgmca_map_hr[PP]
	...
	#temp = temp/1e6
	#imshow(sptpol_gnom);colorbar();show();sys.exit()
	sptpol_gnom = sptpol_gnom/1e6
	from IPython import embed;embed()
	...
	subplot(132);imshow(map1*1e6,vmin =-100,vmax = 100);colorbar()#;show()
	subplot(131);imshow(map2*1e6,vmin =-100,vmax = 100);colorbar()#;show()
	subplot(133);imshow(map3*1e6,vmin =-100,vmax = 100);colorbar();show()
	"""
	j = j+1
	suptitle('Comparing SZ and planck cutouts at random cluster locations')
	k = k +1
	subplot(1,2,1);imshow(map1*1e6,vmin =-100,vmax = 100);colorbar();title('sz_map (%s, %s)'%(ra, dec))
	k = k +1
	subplot(1,2,2);imshow(map2*1e6,vmin =-100,vmax = 100);colorbar();title('planck_map( %s,  %s)'%(ra, dec))
	show()
	#show()
	"""
